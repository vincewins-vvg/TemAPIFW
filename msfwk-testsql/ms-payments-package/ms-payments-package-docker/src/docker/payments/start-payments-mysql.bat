...
helm install appinit ./appinit -n appinitpayments --set env.sqlinit.databaseKey=%DATABASE_KEY% --set env.sqlinit.databaseName=%DATABASE_NAME% --set env.sqlinit.dbdialect=%DIALECT% --set env.sqlinit.dbusername=%DB_USERNAME% --set env.sqlinit.dbpassword=%DB_PASSWORD% --set env.sqlinit.dbconnectionurl=jdbc:mysql://paymentorder-db-service.payments.svc.cluster.local:3306/payments --set env.sqlinit.dbdriver=%DRIVER_NAME% --set env.sqlinit.dbautoupgrade="N"
