...
export SASL_JASS_CONFIG="org.apache.kafka.common.security.plain.PlainLoginModule required username=\"$ConnectionString\" password=\"Endpoint=sb://posql-kafka.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=yUoa/1dlVqghGSUhf3YWUH5v7sbz9stw5ozyk8MaCx8=\";"
...
export KAFKA_SERVER="posql-kafka.servicebus.windows.net:9093"
export SSL_ENABLED="true"
export MAX_POLL_RECORDS=20

export RESOURCE_STORAGE_NAME="paymentordersql"
...
export INGESTER_APP_1="paymentinboxpinkavro"
...
az mysql server create --name %DB_NAME_SPACE% --resource-group %RESOURCE_GROUP_NAME% --location %LOCATION% --admin-user %DB_ADMIN_USERNAME% --admin-password %DB_PASSWORD% --sku-name GP_Gen5_2 
...
az mysql db create --name %DB_NAME% --resource-group %RESOURCE_GROUP_NAME% --server-name %DB_NAME_SPACE%
