...
CREATE PROCEDURE INBOX_PROCEDURE (IN eventId VARCHAR(255),IN eventType varchar(255)) LANGUAGE SQL BEGIN CALL mysql.lambda_async('arn:aws:lambda:eu-west-2:177642146375:function:inbox-sql-handler', CONCAT('{ \"eventId\" : \"', @eventId, '\", \"eventType\" : \"', @eventType, '\"}')); END;
...
CREATE TRIGGER INBOX_TRIGGER AFTER INSERT ON ms_inbox_events FOR EACH ROW BEGIN SELECT  NEW.eventId, NEW.eventType INTO @eventId, @eventType; CALL INBOX_PROCEDURE(@eventId , @eventType); END;
