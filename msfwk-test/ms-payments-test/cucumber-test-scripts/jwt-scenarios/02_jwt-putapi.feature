Feature: 02_jwt-putapi
  ...
  | description | string |Payment ref|
   ...
   #Actual Update via PO PUT Api   
    ...
    Given MS request URI is "v1.0.0/payments/orders/PO~111~222~USD~121"
    
    And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/UpdatePaymentOrderjwt.json"
    When a "PUT" request is sent to MS
    And log all MS response in console
    Then MS response code should be 200
     
    
  Scenario: To verify the responses when authorization header is not passed in PO PUT Api
 ...
 Scenario: To verify the responses when token passed in expired in PO PUT Api
 ...
 Scenario: To verify the responses when token passed in invalid in PO PUT Api
 ...
 Scenario: To verify the responses when null is passed as part of authorization header in PO PUT Api
