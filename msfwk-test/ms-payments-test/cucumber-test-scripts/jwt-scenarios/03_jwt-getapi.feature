Feature: 03_jwt-getapi
  ...
  | description | string |Payment ref|
   ...
   #Actual retrieving via PO GET Api   
    And create a new MS request with code using Restassured arguments "GET_PAYMENTORDER_AUTH_CODE"
    And MS query parameter for Azure env is set to value ""
    #Valid Token is passed in authorization
    And MS request header "Content-Type" is set to "application/json"
    
    Given MS request URI is "v1.0.0/payments/orders/PO~112~222~USD~121"
