Feature: 09_DuplicateQueryParams
  ...
  Scenario: To check PO GET API for pagination with page number query parameter duplicated

    And MS query parameter "pageNumber" is set to value ""
    ...
    Then MS JSON property "items" should contain at most 1 elements


 
  Scenario: To check PO GET API for pagination with page size query parameter duplicated
  
    
    And MS query parameter "pageSize" is set to value ""
    And MS query parameter "pageNumber" is set to value "1"
    And MS query parameter "pageSize" is set to numeric value "2"
