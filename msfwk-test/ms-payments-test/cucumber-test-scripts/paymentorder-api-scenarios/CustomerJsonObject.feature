  Feature: CustomerJsonObject

  Background: To setup the preconfigs
    Given Set the test backgound for PAYMENT_ORDER API

  Scenario Outline: To get customer json object values through api
    Given create a new MS request with code using Restassured arguments "GET_PAYMENTORDERSCURRENCY_AUTH_CODE"
    And MS query parameter for Azure env is set to value ""
    And MS request URI is "v1.0.0/getDynamicType/orders?id=1000&order=customer"
    ...
    Examples: 
      | response                                                          |
      | {"gender": "female","name": "Jany","place": "mexico","age": "25"} |
