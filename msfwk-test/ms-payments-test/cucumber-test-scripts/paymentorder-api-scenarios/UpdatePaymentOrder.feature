...
Background: To set the preconfig for the scenarios
   ...
   Scenario: Update an existing Payment Order
    ...
    Given MS request URI is "payments/orders/PO~100-CBE~222-VVG~USD~433"
    And create a new MS request with code using Restassured argument "UPDATE_PAYMENTORDER_AUTH_CODE"
    And MS request header "Content-Type" is set to "application/json"
    ...
    Given MS request URI is "payments/orders/PO~100-CBE~222-VVG~USD~433"
    And create a new MS request with code using Restassured argument "GET_PAYMENTODER_AUTH_CODE"
