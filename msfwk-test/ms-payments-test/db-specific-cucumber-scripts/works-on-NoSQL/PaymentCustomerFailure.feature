...
Scenario Outline: create customer of same unique ID
	...
	Then MS response code should be 400 
	 And MS JSON response string property key "message" should contain value "Mongodao operation exception while 'saveEntity' onE11000 duplicate key error collection: ms_paymentorder.ms_payment_order_customer index: customerId_1 dup key: { customerId: \"Test2\" }"
    And MS JSON response string property key "code" should contain value "MSF-999"
	
	
	Examples: 
        |payload|
		|{"customerId":"Test2","customerName":"Will","account":"Savings","loanTypes":["education loan"],"dateOfJoining":"2020-08-26"}|
