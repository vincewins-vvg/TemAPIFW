Feature: ErasureOptionsInNullify_Validation
  ...
  | description | string |Payment ref|
   ...
   #Nullify with option value validation 
    ...
    Then MS response code should be 400
     And MS JSON response string property key "message" should equal value "[If optionId is NULLIFY, Optionvalue should be an empty string]"
   ...
   #Nullify with white space and empty string option value validation 
    ...
    And MS JSON response string property key "customerId" should equal value "PO~354-ER10~123-ABC~USD~485"
    ...
    Examples: 
         |payload|
         |{"partyId":"PO~DATEST~CADEST~USD~100","customerId":"PO~354-ER10~123-ABC~USD~485","serviceId":"PARTY","customEntityidentifier":{"currency":"100"},"personalData":[{"purpose":"tax","dataDefinitions":[{"dataEntityName":"PaymentOrder","dataEntityFields":[{"dataEntityFieldName":"paymentDate","dataEntityFieldDataType":"string","erasureOptions":{"optionsId":"NULLIFY","optionsValue":""}}]}]},{"purpose":"card","dataDefinitions":[{"dataEntityName":"PaymentOrder","dataEntityFields":[{"dataEntityFieldName":"amount","dataEntityFieldDataType":"string","erasureOptions":{"optionsId":"NULLIFY","optionsValue":"  "}}]}]}]}|
  ...
  #after nullify,retry with option value with any char(it should not throw error response because of field not available)
    ...
    Examples: 
         |payload|
         |{"partyId":"PO~DATEST~CADEST~USD~100","customerId":"PO~354-ER10~123-ABC~USD~485","serviceId":"PARTY","customEntityidentifier":{"currency":"100"},"personalData":[{"purpose":"tax","dataDefinitions":[{"dataEntityName":"PaymentOrder","dataEntityFields":[{"dataEntityFieldName":"status","dataEntityFieldDataType":"string","erasureOptions":{"optionsId":"ALS","optionsValue":""}}]}]},{"purpose":"card","dataDefinitions":[{"dataEntityName":"PaymentOrder","dataEntityFields":[{"dataEntityFieldName":"paymentDetails","dataEntityFieldDataType":"string","erasureOptions":{"optionsId":"nullifys","optionsValue":"M"}}]}]}]}|
         
    Scenario Outline: Invalid nullify and valid erasure  within same purpose
    ...
    Examples: 
         |payload|
         |{"partyId":"PO~DATEST~CADEST~USD~100","customerId":"PO~354-ER10~123-ABC~USD~485","serviceId":"PARTY","customEntityidentifier":{"currency":"100"},"personalData":[{"purpose":"tax","dataDefinitions":[{"dataEntityName":"PaymentOrder","dataEntityFields":[{"dataEntityFieldName":"paymentReference","dataEntityFieldDataType":"string","erasureOptions":{"optionsId":"NULLIFY"}},{"dataEntityFieldName":"paymentReference","dataEntityFieldDataType":"string","erasureOptions":{"optionsId":"ALS","optionsValue":"P"}}]}]}]}|
         
   Scenario Outline: Valid nullify and erasure within same purpose(it will work as sequencial streaming)
    ...
    Examples: 
         |payload|
         |{"partyId":"PO~DATEST~CADEST~USD~100","customerId":"PO~354-ER10~123-ABC~USD~485","serviceId":"PARTY","customEntityidentifier":{"currency":"100"},"personalData":[{"purpose":"tax","dataDefinitions":[{"dataEntityName":"PaymentOrder","dataEntityFields":[{"dataEntityFieldName":"paymentDetails","dataEntityFieldDataType":"string","erasureOptions":{"optionsId":"NULLIFY","optionsValue":""}},{"dataEntityFieldName":"paymentDetails","dataEntityFieldDataType":"string","erasureOptions":{"optionsId":"als","optionsValue":"p"}}]}]}]}|
    ...
    And create a new MS request with code using Restassured arguments "CREATE_PAYMENTORDER_AUTH_CODE"
    And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/SQL/Report5.json"
    When a "PUT" request is sent to MS
    And log all MS response in console
    Then MS response code should be 200
    And check full response with expected json content from file path "src/test/resources/cucumber-json-payload/GDPR/SQL/ReportResult5.json"      