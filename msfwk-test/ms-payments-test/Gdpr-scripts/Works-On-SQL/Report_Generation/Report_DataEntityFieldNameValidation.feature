Feature: Report_DataEntityFieldNameValidation
  ...
  | description | string |Payment ref|
   ...
   #DataEntityFieldName validation 
    ...
    And MS JSON response string property key "errorMessage" should contain value "[Method not foundcom.temenos.microservice.payments.entity.Card.getPayeeType-com.temenos.microservice.payments.entity.Card.getPayeeType()]"
    And MS JSON response string property key "partyId" should equal value "[PO~DATEST~CADEST~USD~100]"
    And MS JSON response string property key "customerId" should equal value "[PO~348-RG4~123-ABC~USD~485]"
  ...
  Scenario Outline: DataEntityFieldName validation with invalid name in last element
    ...
    And log all MS response in console
    Then MS response code should be 400
    And MS JSON response string property key "errorMessage" should contain value "[Method not foundcom.temenos.microservice.payments.entity.ExchangeRate.getPayeeType-com.temenos.microservice.payments.entity.ExchangeRate.getPayeeType()]" 
    And MS JSON response string property key "errorCode" should equal value "[MSF-703]"
    And MS JSON response string property key "partyId" should equal value "[PO~DATEST~CADEST~USD~100]"
    And MS JSON response string property key "customerId" should equal value "[PO~348-RG4~123-ABC~USD~485]"
    And MS JSON response string property key "serviceId" should equal value "[PARTY]"
