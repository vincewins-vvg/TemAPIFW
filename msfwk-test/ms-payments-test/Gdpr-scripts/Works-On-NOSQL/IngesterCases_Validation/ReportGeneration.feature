Feature: ReportGeneration   
  ...
  Scenario Outline: create customer using post method
	...
	And MS JSON response string property key "customerId" should equal value "IG2"
	And MS JSON response string property key "status" should equal value "Created"

Examples: 
        |payload|
		|{"customerId":"IG2","customerName":"Riya","account":"312-IG2","loanTypes":["education loan","housing loan","car loan"],"dateOfJoining":"2020-08-26"}|
  ...
  | value | string |paytesterig2|
  | description | string |Payment ref|
    
    And MS request URI is "v1.0.0/payments/orders" 
	And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/CreatePOWithExtensionData_IngesterErasure4.json"
	When a "POST" request is sent to MS 
	And log all MS response in console 
	Then MS response code should be 200 
 ...
 Scenario: Send Data to topic and validate Report generation functionality
    ...
    Given Set the test backgound for PAYMENT_ORDER API
    Given Set the Testcase id MS-Test-PO-ReportIngester-001 for company GB0010001
  
    When Send Data to Topic ms-paymentorder-inbox-topic from file cucumber-json-payload/GDPR/NOSQL/IngesterReport.json and authorizationFieldName headers.Authorization for Application PAYMENT_ORDER
    ...
    Then Set the following data criteria
      ...
      | MS-Test-PO-ReportIngester-001| eventId    | eq       | string   | 9dc99a2c-c3ee-4393-8e58-c4ef920ef02711REPORT |
      | MS-Test-PO-ReportIngester-001| eventType  | eq       | string   | ms-paymentorder.ExecuteSubjectAccessRequest |
    ...
    And Validate the below details from the db table ms_inbox_events and check no of record is 1
      ...
      | MS-Test-PO-ReportIngester-001| status    | PROCESSED   |
      
     # Then check if json data with correlation id 9dc99a2c-c3ee-4393-8e58-c4ef920ef02711REPORT and subject commandprocessed is present in topic ms-eventstore-inbox-topic with count should be equal to 1
     
     # Then check if json data with correlation id 9dc99a2c-c3ee-4393-8e58-c4ef920ef02711REPORT and subject event is present in topic ms-eventstore-inbox-topic with count should be equal to 1
