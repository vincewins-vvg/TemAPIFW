...
Examples: 
  ...
  | description | string |Payment ref|
   ...
   #DataEntityFieldName validation 
    ...
    And MS JSON response string property key "errorMessage" should contain value "[Method not foundcom.temenos.microservice.paymentorder.entity.ExchangeRate.getCurrency-com.temenos.microservice.paymentorder.entity.ExchangeRate.getCurrency()]"
  ...
  Scenario Outline: DataEntityFieldName validation with invalid name in last element
    ...
    And log all MS response in console
    Then MS response code should be 400
    And MS JSON response string property key "errorMessage" should contain value "[Method not foundcom.temenos.microservice.paymentorder.entity.Card.getAmount-com.temenos.microservice.paymentorder.entity.Card.getAmount()]" 
    And MS JSON response string property key "errorCode" should equal value "[MSF-703]"
    And MS JSON response string property key "partyId" should equal value "[PO~DATEST~CADEST~USD~100]"
    And MS JSON response string property key "customerId" should equal value "[ER4]"
    And MS JSON response string property key "serviceId" should equal value "[PARTY]"
