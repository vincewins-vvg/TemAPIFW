Feature: ErasureAndNullify_Validation
  ...
  Scenario Outline: create customer using post method
	...
	And MS JSON response string property key "customerId" should equal value "ER9"
	And MS JSON response string property key "status" should equal value "Created"

Examples: 
        |payload|
		|{"customerId":"ER9","customerName":"Riya","account":"353-ER9","loanTypes":["education loan","Personal loan","Car loan","Housing loan","Bike loan"],"dateOfJoining":"2020-08-26"}|
  ...
  | description | string |Payment ref|
	...
	And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/CreatePOWithExtensionData_Erasure2.json"
	When a "POST" request is sent to MS 
	And log all MS response in console 
	Then MS response code should be 201 
   ...
   # Erasing the data before nullify
    ...
    And create a new MS request with code using Restassured arguments "CREATE_PAYMENTORDER_AUTH_CODE"
    And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/Erasure2.json"
...
#And Validate the below details from the db table card and check no of record is 1
    ...
    And create a new MS request with code using Restassured arguments "CREATE_PAYMENTORDER_AUTH_CODE"
    And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/Report2.json"
    When a "PUT" request is sent to MS
    And log all MS response in console
    Then MS response code should be 200
    And check full response with expected json content from file path "src/test/resources/cucumber-json-payload/GDPR/NOSQL/ReportResult2.json"
    ...
    And create a new MS request with code using Restassured arguments "CREATE_PAYMENTORDER_AUTH_CODE"
    And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/Nullify1.json"
    When a "PUT" request is sent to MS
    And log all MS response in console
    Then MS response code should be 200
    And MS JSON property "partyId" should exist
    ...
    And create a new MS request with code using Restassured arguments "CREATE_PAYMENTORDER_AUTH_CODE"
    And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/Report3.json"
    When a "PUT" request is sent to MS
    And log all MS response in console
    Then MS response code should be 200
    And check full response with expected json content from file path "src/test/resources/cucumber-json-payload/GDPR/NOSQL/ReportResult3.json"
    ...
    And Validate the below details from the db table ms_payment_order and check no of record is 1
      | TestCaseID       | ColumnName       | ColumnValue |
      | MS-Test-ER-20    | paymentOrderId   | PO~353-ER9~321-ZXC~INR~125 |
      | MS-Test-ER-20    | amount           |  |
      | MS-Test-ER-20    | status           |  | 
      | MS-Test-ER-20    | amount           |  |
      | MS-Test-ER-20    | currency         | INR |
      | MS-Test-ER-20    | paymentDate      |  |
      | MS-Test-ER-20    | paymentDetails   |  |
      | MS-Test-ER-20    | paymentReference |  | 
