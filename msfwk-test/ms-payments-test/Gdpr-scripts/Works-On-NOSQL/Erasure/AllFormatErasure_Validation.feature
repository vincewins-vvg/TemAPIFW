Feature: AllFormatErasure_Validation
  ...
  Scenario Outline: create customer using post method
	...
	And MS JSON response string property key "customerId" should equal value "ER8"
	And MS JSON response string property key "status" should equal value "Created"

Examples: 
        |payload|
		|{"customerId":"ER8","customerName":"Riya","account":"352-ER8","loanTypes":["education loan","housing loan","car loan"],"dateOfJoining":"2020-08-26"}|
  ...
  | value | string |paytesterasure8|
  | description | string |Payment ref|
    
    And MS request URI is "v1.0.0/payments/orders" 
	And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/CreatePOWithExtensionData_Erasure1.json"
	When a "POST" request is sent to MS 
	And log all MS response in console 
	Then MS response code should be 200 
   ...
   # AllFormatErasure_Validation
    Scenario: AllFormatErasure_Validation
    ...
    And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/Erasure1.json"
    ...
    Then Set the following data criteria
      ...
      | MS-Test-ER-20    | paymentOrderId    | eq       | string   | PO~352-ER8~321-ZXC~INR~125 |
    ...
    And Validate the below details from the db table ms_payment_order and check no of record is 1
      ...
      | MS-Test-ER-20    | paymentOrderId   | PO~352-ER8~321-ZXC~INR~125 |
      | MS-Test-ER-20    | amount           | 999 |
      | MS-Test-ER-20    | status           | SSSSSSSSS | 
      | MS-Test-ER-20    | amount           | 999 |
      #| MS-Test-ER-20    | currency         | NNN | (commented because of enum type retrieve for PO GET api)
      #| MS-Test-ER-20    | paymentDate      | 9999-12-31 |(format differs in mongodb)
...
#And Validate the below details from the db table card and check no of record is 1
    ...
    And MS request URI is "v1.0.0/party/personalData/reports/SAR/requests/111.ERASURE"
    And the MS request body is set to the contents of "src/test/resources/cucumber-json-payload/GDPR/NOSQL/Report1.json"
    When a "PUT" request is sent to MS
    And log all MS response in console
    Then MS response code should be 200
    And check full response with expected json content from file path "src/test/resources/cucumber-json-payload/GDPR/NOSQL/ReportResult1.json"
    
     #Erasing the same entry again to check duplication
     Scenario: Erasing the same entry again to check duplication
  ...
  Scenario: Retrieve the above erased duplicate entry to check any masking happened again
