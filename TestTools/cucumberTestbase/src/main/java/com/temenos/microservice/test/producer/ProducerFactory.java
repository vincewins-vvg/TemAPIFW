...
public class ProducerFactory {
	...
	public static StreamProducer createStreamProducer(String producerName, String streamVendor) {
		if ("kinesis".equals(streamVendor)) {
			...
			if ("true".equals(Environment.getEnvironmentVariable("temn.msf.kafka.ssl.enabled", "false"))) {
				...
				props.put("ssl.keystore.password", Environment.getEnvironmentVariable(
						IngesterConfigProperty.STREAM_KAFKA_SSL_KEYSTORE_PASSWORD.getName(), ""));
				...
				props.put("ssl.truststore.location", Environment.getEnvironmentVariable(
						IngesterConfigProperty.STREAM_KAFKA_SSL_TRUSTSTORE_LOCATION.getName(), ""));
				props.put("ssl.truststore.password", Environment.getEnvironmentVariable(
						IngesterConfigProperty.STREAM_KAFKA_SSL_TRUSTSTORE_PASSWORD.getName(), ""));
				props.put("security.protocol", Environment.getEnvironmentVariable(
						IngesterConfigProperty.STREAM_KAFKA_SECURITY_PROTOCOL.getName(), "SSL"));
			}
