...
REM Description 	: Specifies the name of Images for api ,ingester,inboxoutbox, dbinit that are  pushed to external repositories,
...
REM Name			: ImagePullSecret,dbinit_Image_Pull_Secret
...
helm upgrade paymentorder ./svc -n paymentorder --set env.database.POSTGRESQL_CONNECTIONURL=jdbc:postgresql://po-postgresqldb-service.postgresql.svc.cluster.local:5432/paymentorderdb --set env.database.MONGODB_DBNAME=%database_Name% --set env.database.DATABASE_KEY=%database_Key% --set image.paymentorderapi.repository=%apiImage% --set image.paymentorderingester.repository=%ingesterImage% --set image.paymentorderscheduler.repository=%schedulerImage% --set image.paymentorderinboxoutbox.repository=%inboxoutboxImage% --set image.fileingester.repository=%fileingesterImage% --set image.schemaregistry.repository=%schemaregistryImage% --set imagePullSecrets=%ImagePullSecret% --set image.tag=%tag% --set env.genericconfig.basepath=%gc_Base_Path% --set env.kafka.kafkabootstrapservers=%kafka_Bootstrap_Servers%  --set env.name=%env_Name% --set configmap.location=%config_Location% --set rollingupdate.enabled=%rollingUpdate% --set rollingupdate.enabled=%rolling_Update% --set rollingupdate.api.maxsurge=%api_MaxSurge% --set rollingupdate.api.maxunavailable=%api_MaxUnavailable% --set rollingupdate.ingester.maxsurge=%ingester_MaxSurge% --set rollingupdate.ingester.maxunavailable=%ingester_MaxUnavailable% --set pit.JWT_TOKEN_ISSUER=%Jwt_Token_Issuer% --set pit.JWT_TOKEN_PRINCIPAL_CLAIM=%Jwt_Token_Principal_Claim% --set pit.ID_TOKEN_SIGNED=%Id_Token_Signed% --set pit.JWT_TOKEN_PUBLIC_KEY_CERT_ENCODED=%Jwt_Token_Public_Key_Cert_Encoded% --set pit.JWT_TOKEN_PUBLIC_KEY=%Jwt_Token_Public_Key% --set pit.ms_security_tokencheck_enabled=%ms_security_tokencheck_enabled%
