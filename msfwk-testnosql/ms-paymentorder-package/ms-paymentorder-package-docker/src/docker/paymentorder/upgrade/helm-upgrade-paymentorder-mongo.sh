...
# Default value   : mongodb+srv://user01:user01@mongodb01.jx2tl.mongodb.net/test
...
# Default Value    : servicerequestusr
...
# Default Value    : servicerequestpass
...
export env_Name=""
# Name				: servicerequestImagePullSecret,dbinit_Image_Pull_Secret
...
# Description 	: location specfied for the configmap.yaml
# Default Value 	: svc/ms-servicerequest-configmap.yaml
export config_Location="svc/ms-servicerequest-configmap.yaml"
...
helm upgrade paymentorder ./svc -n paymentorder --set env.database.MONGODB_DBNAME=$database_Name --set env.database.POSTGRESQL_CONNECTIONURL=jdbc:postgresql://ms-paymentorder-postgresqldb-service.postgresql.svc.cluster.local:5432/ms_paymentorder --set env.database.DATABASE_KEY=$database_Key  --set env.database.MONGODB_CONNECTIONSTR=$db_Connection_Url --set image.paymentorderapi.repository=$apiImage --set image.paymentorderingester.repository=$ingesterImage --set image.paymentorderscheduler.repository=$schedulerImage --set image.paymentorderinboxoutbox.repository=$inboxoutboxImage --set image.fileingester.repository=$fileingesterImage --set image.schemaregistry.repository=$schemaregistryImage --set imagePullSecrets=$ImagePullSecret --set image.tag=$tag --set env.genericconfig.basepath=$gc_Base_Path --set env.kafka.kafkabootstrapservers=$kafka_Bootstrap_Servers --set env.name=$env_Name --set rollingupdate.enabled=$rollingUpdate --set rollingupdate.enabled=$rolling_Update --set rollingupdate.api.maxsurge=$api_MaxSurge --set rollingupdate.api.maxunavailable=$api_MaxUnavailable --set rollingupdate.ingester.maxsurge=$ingester_MaxSurge --set rollingupdate.ingester.maxunavailable=$ingester_MaxUnavailable --set pit.JWT_TOKEN_ISSUER=$Jwt_Token_Issuer --set pit.JWT_TOKEN_PRINCIPAL_CLAIM=$Jwt_Token_Principal_Claim --set pit.ID_TOKEN_SIGNED=$Id_Token_Signed --set pit.JWT_TOKEN_PUBLIC_KEY_CERT_ENCODED=$Jwt_Token_Public_Key_Cert_Encoded --set pit.JWT_TOKEN_PUBLIC_KEY=$Jwt_Token_Public_Key --set pit.ms_security_tokencheck_enabled=$ms_security_tokencheck_enabled --set image.appinit.repository=$APP_INIT_IMAGE
